...
## Example
关于下图的示例中，首先，我在 `nox/scripts/poker/` 目录下，通过 `nox system create -s ace` 命令创建了一个符合规范的 shell 脚本 `ace.sh`。在开发完具体实现后，通过 `nox system build` 编译生成自动补全逻辑。然后执行 `source ~/.zshrc` 更新自动补全逻辑。最后，输入并结合 Tab 补全，得到上图中的效果。
